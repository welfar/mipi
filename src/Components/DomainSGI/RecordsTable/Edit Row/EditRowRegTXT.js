...
export const EditRow = ({ index, setEditMode, subdomain, purpose }) => {
  ...
  const handleSave = (e) => {
    ...
    if (editInfoRow.prior === "" || editInfoRow.prior === null) {
      alert("No se puede crear un registro sin prioridad");
      return;
    }
    ...
    const rrsets = {
      ...
      prior: editInfoRow.prior,
    };
    Swal.fire({
      title: "¿Estas seguro?",
      html:
        `<p>Se realizara el cambio del registro <b>${editInfoRow.subdomain}</b> con apuntamiento hacia : <b>${editInfoRow.purpose}</b> ?</p>` +
  ...
  return (
    <>
      ...
      >
        ...
        <div className="row__content" style={{ width: "50%" }}>
          <input
